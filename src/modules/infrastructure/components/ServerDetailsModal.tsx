      ...
      <div className="bg-gradient-to-br from-secondary-600 to-secondary-700 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-slide-up">
        <div className="sticky top-0 bg-gradient-to-br from-secondary-600 to-secondary-700 border-b border-secondary-200 p-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{server.operating_system}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="card p-4">
              <p className="text-sm text-gray-400 mb-2">Dirección IP</p>
              <p className="font-mono font-bold text-white">{server.ip_address}</p>
            </div>
            <div className="card p-4">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-secondary-600/50 rounded-lg p-4">
                ...
                <p className="text-2xl font-bold text-white">{server.cpu_cores}</p>
              </div>
              <div className="bg-secondary-600/50 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">RAM</p>
                <p className="text-2xl font-bold text-white">{server.ram_gb} GB</p>
              </div>
              <div className="bg-secondary-600/50 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Disco</p>
                <p className="text-2xl font-bold text-white">{server.disk_gb} GB</p>
              </div>
              <div className="bg-secondary-600/50 rounded-lg p-4">
                <p className="text-xs text-gray-400 mb-1">Uptime</p>
                <p className="text-2xl font-bold text-white">{Math.floor(server.uptime_hours / 24)}d</p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-gray-300 font-semibold">Uso de CPU</span>
                  <span className="font-bold text-white">{server.cpu_usage_percent}%</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-gray-300 font-semibold">Uso de RAM</span>
                  <span className="font-bold text-white">{server.ram_usage_percent}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-4">
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-gray-300 font-semibold">Uso de Disco</span>
                  <span className="font-bold text-white">{server.disk_usage_percent}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-4">
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3">
              <div>
                <p className="text-sm text-gray-400">Fecha de Instalación</p>
                <p className="font-bold text-white">{formatDate(server.installation_date)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Último Mantenimiento</p>
                <p className="font-bold text-white">{formatDate(server.last_maintenance)}</p>
              </div>
            </div>
          </div>
            ...
            <div className="card p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {server.services_running.map((service, idx) => (
                  <span key={idx} className="px-4 py-2 bg-primary-900/20 text-blue-700 text-sm font-medium rounded-full">
                    {service}
                  </span>
                ))}
              </div>
