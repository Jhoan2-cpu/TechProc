        ...
        <div className="sticky top-0 bg-white border-b border-secondary-200 p-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-secondary-600">Versión {software.version}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-secondary-50 rounded-lg p-4">
                <p className="text-xs text-secondary-600 mb-1">Categoría</p>
                <span className="inline-block px-3 py-2 rounded-full text-sm font-bold bg-blue-100 text-blue-700">
                  {software.category}
                </span>
              </div>
              <div className="bg-secondary-50 rounded-lg p-4">
                <p className="text-xs text-secondary-600 mb-1">Proveedor</p>
                <p className="text-lg font-bold text-secondary-900">{software.vendor}</p>
              </div>
              <div className="bg-secondary-50 rounded-lg p-4">
                <p className="text-xs text-secondary-600 mb-1">Versión Actual</p>
                <p className="text-lg font-bold text-secondary-900">{software.version}</p>
              </div>
              <div className="bg-secondary-50 rounded-lg p-4">
                <p className="text-xs text-secondary-600 mb-1">Actualizaciones Automáticas</p>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-secondary-600">Fecha de Instalación</span>
                <span className="font-bold text-secondary-900">{formatDate(software.installation_date)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-secondary-600">Última Actualización</span>
                <span className="font-bold text-secondary-900">{formatDate(software.last_update)}</span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-secondary-900">{formatDate(software.support_until)}</span>
              </div>
            </div>
          </div>
          ...
          {software.license_id && (
            <div className="card p-6 bg-purple-50 border-purple-200">
              <h4 className="text-lg font-heading font-bold text-secondary-900 mb-3 flex items-center gap-2">
                <FontAwesomeIcon icon={faKey} className="text-purple-600" />
                Licencia Asociada
              </h4>
              <div className="bg-white rounded-lg p-4 border-2 border-purple-200">
                <p className="text-secondary-700">
                  ID de Licencia: <span className="font-bold text-secondary-900">#{software.license_id}</span>
                </p>
            ...
            {software.server_ids.length > 0 ? (
              <div>
                <p className="text-secondary-600 mb-3">
                  Este software está instalado en <span className="font-bold text-secondary-900">{software.server_ids.length}</span> servidor(es)
                </p>
                <div className="flex flex-wrap gap-2">
                  {software.server_ids.map((serverId) => (
                    <div
                      key={serverId}
                      className="px-4 py-2 bg-primary-100 text-primary-700 font-medium rounded-lg flex items-center gap-2"
                    >
