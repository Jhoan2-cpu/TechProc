          ...
          {loadingClasses ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="mt-3 text-sm text-gray-400">Cargando grupos...</p>
              </div>
            </div>
          ) : groups.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-400 text-lg">No hay grupos creados para este curso</p>
              <p className="text-gray-500 text-sm mt-2">Haz clic en "Crear Grupo" para comenzar</p>
            </div>
          ) : (
            <div className="space-y-6">
              {/* Tabs de Grupos */}
              <div className="flex items-center gap-3 border-b border-secondary-500 overflow-x-auto">
                {groups.map((group, index) => (
                  <button
                    key={group.id}
                    onClick={() => setActiveGroupTab(index)}
                    className={`px-4 py-3 font-medium text-sm whitespace-nowrap transition-all duration-300 ${
              ...
              {groups[activeGroupTab] && (
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h4 className="text-lg font-semibold text-white mb-1">
                          {groups[activeGroupTab].name}
                        </h4>
                        <p className="text-sm text-gray-400">
                          <span className="font-medium text-primary-400">{groups[activeGroupTab].code}</span>
                          {' • '}
                          {formatDate(groups[activeGroupTab].start_date)} - {formatDate(groups[activeGroupTab].end_date)}
                        </p>
                      </div>
                      <button
                        onClick={() => {
                          setSelectedGroupId(groups[activeGroupTab].id);
                  ...
                  {/* Clases del grupo activo */}
                  {(() => {
                    const groupClasses = getClassesByGroup(groups[activeGroupTab].id);

                    if (groupClasses.length === 0) {
                      return (
                        <div className="text-center py-12">
                          <p className="text-gray-400 text-lg">No hay clases programadas para este grupo</p>
